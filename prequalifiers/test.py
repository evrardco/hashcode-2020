    ...
    def successors(self):
        ...
        for i in range(len(self.used)):
            for j in range(len(self.unused)):
                if (j, i) in self.swaps:
                    continue
                yield self.swap(i, j)

    def swap(self, i, j):
        s = State(self.count, self.used, self.unused)
        s.count -= s.used[i]

        s.count += s.unused[j]
        s.swaps = self.swaps + [(i, j)]

        return s
    def __hash__(self):
        return hash(functools.reduce(lambda x, y: x[0] + y[0] + x[1] + y[1], self.swaps))
    
    def __eq__(self, other):
        s1 = functools.reduce(lambda x, y: x[0] + y[0] + x[1] + y[1], self.swaps)
        s2 = functools.reduce(lambda x, y: x[0] + y[0] + x[1] + y[1], other.swaps)
        return s1 == s2
